...
class EducationalPlatformsViewController: UIViewController {
   ...
   @IBAction func edutopiaButtonClicked(_ sender: Any) {
    ...
    @IBAction func codecademyButtonClicked(_ sender: Any) {
        if let url = NSURL(string: "https://www.codecademy.com/") {
            UIApplication.shared.openURL(url as URL)
        }
    }
    
    @IBAction func girlswhocodeButtonClicked(_ sender: Any) {
        if let url = NSURL(string:"https://girlswhocode.com/") {
            UIApplication.shared.openURL(url as URL)
        }
    }
    
    @IBAction func codeorgButtonClicked(_ sender: Any) {
        if let url = NSURL(string: "https://code.org/learn") {
            UIApplication.shared.openURL(url as URL)
        }
    }
    
    @IBAction func khanAcademyButtonClicked(_ sender: Any) {
        if let url = NSURL(string: "https://www.khanacademy.org/") {
            UIApplication.shared.openURL(url as URL)
        }
    }
    
    
    @IBAction func SociWomenEngiButtonClicked(_ sender: Any) {
        if let url = NSURL(string: "https://swe.org/") {
            UIApplication.shared.openURL(url as URL)
        }
    }
    
    
    @IBAction func NatiGirCollabProj(_ sender: Any) {
        if let url = NSURL(string:"https://ngcproject.org/") {
            UIApplication.shared.openURL(url as URL)
        }
    }
